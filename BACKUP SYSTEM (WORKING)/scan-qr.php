...
// Validate the token
$stmt = $conn->prepare("SELECT id FROM qr_tokens WHERE student_id = ? AND token = ? LIMIT 1");
...
if ($result->num_rows === 0) {
    // Invalid token
    header('Location: login.php?error=invalid_qr');
    exit();
}
...
if (!isset($_SESSION['user_id'])) {
    ...
    $returnUrl = urlencode('adminstudentviolation.php?id=' . $studentId);
    header('Location: login.php?return=' . $returnUrl . '&token=' . urlencode($token) . '&qr_id=' . $studentId);
    exit();
}

// User is logged in - now capture location and store in cookies before redirecting
$conn->close();
