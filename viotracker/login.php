...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!isset($_POST['csrf_token']) || $_POST['csrf_token'] !== $_SESSION['csrf_token']) {
        ...
        if (empty($email) || !filter_var($email, FILTER_VALIDATE_EMAIL)) {
            ...
            // Query the teacher table for the user
            ...
            if (!$stmt) {
                ...
                if (!$stmt->execute()) {
                    ...
                    if ($result->num_rows === 0) {
                        ...
                        if ($passwordMatch) {
                            // Verify user type matches position
                            $userPosition = strtolower($user['position']);
                            $requestedType = strtolower($user_type);
                            
                            if (($requestedType === 'admin' && $userPosition === 'admin') || 
                                ($requestedType === 'teacher' && in_array($userPosition, ['teacher', 'head teacher', 'department head']))) {
