...
foreach ($violations as $violation) {
    ...
    $dupCheckSql = "SELECT id FROM violation WHERE LOWER(title) = LOWER('" . $conn->real_escape_string($title) . "')";
    $dupCheckResult = $conn->query($dupCheckSql);
    
    if ($dupCheckResult && $dupCheckResult->num_rows > 0) {
        $errors[] = "Skipped duplicate: \"$title\" already exists in database";
