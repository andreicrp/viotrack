...
function createTrendChart() {
    ...
    // Calculate totals and averages for insights
    const totalMinor = minorData.reduce((a, b) => a + b, 0);
    const totalSerious = seriousData.reduce((a, b) => a + b, 0);
    const totalMajor = majorData.reduce((a, b) => a + b, 0);
    const totalViolations = totalMinor + totalSerious + totalMajor;
    ...
    try {
                ...
                datasets: [
                    {
                        ...
                        segment: {
                            borderDash: (ctx) => ctx.p0DataIndex === ctx.p1DataIndex ? [5, 5] : []
                        }
                    },
                    {
                        label: 'Serious',
            ...
            options: {
                ...
                plugins: {
                    ...
                    tooltip: {
                        ...
                        borderWidth: 1,
                        displayColors: true,
                        titleFont: { size: 14, weight: 'bold', family: "'Inter', 'Segoe UI', sans-serif" },
                        ...
                        callbacks: {
                            ...
                            label: function(context) {
                                ...
                                return context.dataset.label + ': ' + value + ' violation' + (value !== 1 ? 's' : '');
                            },
                            afterLabel: function(context) {
                                if (context.datasetIndex === 2) {
                                    // After Major, show total for this date
                                    let total = 0;
            ...
            plugins: [{
                afterDatasetsDraw: function(chart) {
                    ...
                    if (chart.data.datasets[0].data.length > 1) {
                        ...
                        // Add subtle trend arrows or indicators
                        chart.data.datasets.forEach((dataset, datasetIndex) => {
                            ...
                            if (data.length >= 2) {
                                ...
                                if (firstValue !== null && lastValue !== null) {
                                    ...
                                    // Just for visual reference - don't draw, let tooltip handle it
                                }
                            }
                        });
                    }
        ...
        console.log('Trend stats:', { totalMinor, totalSerious, totalMajor, totalViolations });
        return true;
    } catch (error) {
...
function createDonutChart() {
    ...
    // Get data from PHP - array is in order: [minor, serious, major]
    ...
    // Extract counts - ensure we're getting the correct values from the array
    let minorCount = 0;
    let seriousCount = 0;
    let majorCount = 0;
    
    // Handle both object keys and array indices
    if (Array.isArray(violationPercentage)) {
        // Array format from PHP: [minor, serious, major]
        minorCount = parseInt(violationPercentage[0]?.count) || 0;
        seriousCount = parseInt(violationPercentage[1]?.count) || 0;
        majorCount = parseInt(violationPercentage[2]?.count) || 0;
    }
    ...
    try {
        ...
        donutChartInstance = new Chart(ctx, {
            ...
            plugins: [{
                afterDatasetsDraw: function(chart) {
                    // Draw percentage labels in center/near edges
                    const centerX = chart.chartArea.left + chart.chartArea.width / 2;
                    const centerY = chart.chartArea.top + chart.chartArea.height / 2;
                    const innerRadius = chart.chartArea.width / 4;
                    const outerRadius = chart.chartArea.width / 2.2;
                    const avgRadius = (innerRadius + outerRadius) / 2;
                    
                    chart.ctx.font = 'bold 13px "Inter", "Segoe UI", sans-serif';
                    ...
                    datasetData.forEach((value, index) => {
                        ...
                        const labelX = centerX + Math.cos(labelAngle) * avgRadius;
                        const labelY = centerY + Math.sin(labelAngle) * avgRadius;
                        ...
                        chart.ctx.fillText(percentage + '%', labelX, labelY);
...
// Calendar quick navigation helpers
function goToToday() {
...
function highlightToday() {
    setTimeout(() => {
        const days = document.querySelectorAll('.day.today');
        days.forEach(day => {
            day.style.animation = 'pulse 1s ease-in-out';
        });
    }, 100);
}
...
function renderCalendar() {
    ...
    // Add days of the month with enhanced interaction
    for (let day = 1; day <= daysInMonth; day++) {
        ...
        if (dayMeetings.length > 0) {
            ...
            dayEl.title = `${dayMeetings.length} meeting${dayMeetings.length > 1 ? 's' : ''}`;
        }
        
        dayEl.textContent = day;
        dayEl.onclick = (e) => openMeetingPanel(day, dateKey, e);
        dayEl.addEventListener('mouseenter', () => {
            if (dayMeetings.length > 0 && !dayEl.classList.contains('selected')) {
                dayEl.style.transform = 'translateY(-4px)';
            }
        });
        ...
        // Add meeting dots indicator with animation
        if (dayMeetings.length > 0) {
    ...
    // Add month navigation keyboard support
...
function openMeetingPanel(day, dateKey, e) {
    ...
    const dayElement = e?.target.closest('.day') || document.querySelector(`[data-date="${dateKey}"]`);
    if (dayElement) {
...
function renderMeetingList() {
    ...
    if (!selectedDate || !selectedDate.dateKey) {
        ...
        meetingList.innerHTML = '<div class="no-meetings"><i class="fas fa-inbox" style="font-size: 32px; color: #d1d5db; margin-bottom: 12px;"></i><div>Please select a date first</div></div>';
        return;
    }
    ...
    if (dayMeetings.length === 0) {
        meetingList.innerHTML = `
            <div class="no-meetings">
                <div style="font-size: 42px; margin-bottom: 12px; color: #d1d5db;">
                    <i class="fas fa-calendar-check"></i>
                </div>
                <div style="font-size: 15px; font-weight: 600; color: #6b7280;">No meetings scheduled</div>
                <div style="font-size: 13px; color: #9ca3af; margin-top: 8px;">This day is clear</div>
            </div>
        `;
        return;
    }
    
    meetingList.innerHTML = dayMeetings.map((meeting, index) => `
        <div class="meeting-item" style="animation-delay: ${index * 0.05}s;">
            <div class="meeting-info">
                ...
                <div class="meeting-details">
                    ...
                    <div style="font-size: 12px; color: #8b5cf6; font-weight: 600; margin-bottom: 6px; display: flex; align-items: center; gap: 6px;">
                        <i class="fas fa-file-alt" style="font-size: 11px;"></i> ${escapeHtml(meeting.type || 'Manual Entry')}
                    </div>
                    <span class="meeting-time" style="background: none; padding: 0; margin: 0; display: flex; align-items: center; gap: 6px;">
                        <i class="fas fa-clock" style="font-size: 12px; color: #9ca3af;"></i> ${escapeHtml(meeting.time)}
                    </span>
                    ${meeting.phone ? `<span class="meeting-phone" style="background: none; padding: 0; margin-top: 6px; display: flex; align-items: center; gap: 6px;">
                        <i class="fas fa-mobile-alt" style="font-size: 12px;"></i> ${escapeHtml(meeting.phone)}
                    </span>` : ''}
                    ${meeting.reason ? `<div class="meeting-reason"><i class="fas fa-comment" style="font-size: 11px; margin-right: 4px;"></i>${escapeHtml(meeting.reason)}</div>` : ''}
                </div>
            </div>
            <div class="meeting-item-actions">
                <button class="icon-btn small" onclick="openMeetingModal(${meeting.id})" title="View Options">
    ...
    const html = `
        <div style="padding: 20px;">
            ...
            <div style="display: flex; gap: 8px;">
                <button onclick="updateMeetingStatus(${id}, document.getElementById('statusSelect').value)" style="flex: ${window.isAdmin ? '0.6' : '1'}; padding: 10px 16px; background: #6366f1; color: white; border: none; border-radius: 8px; font-weight: 600; cursor: pointer; transition: all 0.2s;" onmouseover="this.style.background='#4f46e5'" onmouseout="this.style.background='#6366f1'">
                    <i class="fas fa-check"></i> Update Status
                </button>
                ${window.isAdmin ? `<button onclick="editMeetingFromModal(${id})" style="flex: 0.6; padding: 10px 16px; background: #6366f1; color: white; border: none; border-radius: 8px; font-weight: 600; cursor: pointer; transition: all 0.2s;" onmouseover="this.style.background='#4f46e5'" onmouseout="this.style.background='#6366f1'">
                    <i class="fas fa-edit"></i> Edit
                </button>` : ''}
                ${window.isAdmin ? `<button onclick="deleteMeetingFromModal(${id})" style="flex: 0.6; padding: 10px 16px; background: #ef4444; color: white; border: none; border-radius: 8px; font-weight: 600; cursor: pointer; transition: all 0.2s;" onmouseover="this.style.background='#dc2626'" onmouseout="this.style.background='#ef4444'">
...
function addOrUpdateMeeting() {
    ...
    if (editingMeetingId) {
        body.id = parseInt(editingMeetingId);
    }
        ...
        if (data.success) {
            ...
            if (editingMeetingId) {
                meetings[selectedDate.dateKey] = meetings[selectedDate.dateKey].map(m => 
                    m.id === parseInt(editingMeetingId) ? { ...m, name, time, reason } : m
                );
            } else {
                const newMeeting = { 
                    id: data.report_id, 
