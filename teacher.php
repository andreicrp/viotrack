...
// Fetch teachers from database using prepared statement (exclude admins)
$sql = "SELECT t.id, t.fname, t.mname, t.lname, t.email, t.password, t.position, t.image, 
        CASE WHEN a.id IS NOT NULL THEN CONCAT(t.position, ', Adviser') ELSE t.position END as display_position
        FROM teacher t
        LEFT JOIN adviser a ON t.id = a.tid
        WHERE t.position != 'admin'
        ORDER BY t.id DESC";
        ...
        <div class="table-container">
            <table class="teacher-table" id="teacherTable">
                ...
                <tbody>
                        ...
                        <tr data-teacher-id="<?php echo (int)$teacher['id']; ?>">
                            ...
                            <td>
                                <span class="role-badge" style="background: #dbeafe; color: #1e40af; padding: 6px 12px; border-radius: 6px; font-size: 12px; font-weight: 600;">
                                    <?php echo htmlspecialchars($teacher['display_position'] ?? 'Teacher', ENT_QUOTES, 'UTF-8'); ?>
                                </span>
                            </td>
                            <td>
                            ...
                            <td>
                                <div class="action-buttons">
                                    ...
                                    <button class="btn btn-edit" onclick="appointAsAdviser(<?php echo (int)$teacher['id']; ?>, '<?php echo htmlspecialchars($teacher['fname'] . ' ' . $teacher['lname'], ENT_QUOTES); ?>')" <?php echo (strpos($teacher['display_position'], 'Adviser') !== false ? 'disabled style="opacity: 0.5; cursor: not-allowed;" title="Teacher is already assigned as an adviser"' : ''); ?>>
                                        <i class="fas fa-star"></i> Appoint as Adviser
                                    </button>
                    ...
                    <?php else: ?>
                        <tr>
                            <td colspan="5" style="text-align: center; padding: 20px;">No teachers found</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
...
function appointAsAdviser(id, name) {
    if (!id || id <= 0) {
        alert('Invalid teacher ID');
        return;
    }
        ...
        .then(data => {
            if (data.success) {
                if (data.available && data.available.length > 0) {
                    openAppointAdviserModal(id, name, data.available);
                } else {
                    alert('No available grade/section combinations. All classes already have advisers assigned.');
                }
            } else {
                alert('Error loading available classes: ' + (data.message || 'Unknown error'));
            }
        })
        .catch(error => {
            console.error('Error:', error);
...
function openAppointAdviserModal(id, name, availableCombinations) {
    ...
    availableCombinations.forEach(combo => {
        // Format grade - remove "Grade " prefix if it exists
        const gradeDisplay = combo.grade.toString().includes('Grade') ? combo.grade : combo.grade;
        classOptions += `<option value="${gradeDisplay}|${combo.section}">Grade ${gradeDisplay} - Section ${combo.section}</option>`;
    });

    // Create and display the modal
    const modal = document.createElement('div');
    modal.id = 'appointAdviserModal';
    modal.innerHTML = `
            ...
            <div style="background: white; border-radius: 12px; padding: 30px; max-width: 600px; width: 100%; box-shadow: 0 4px 20px rgba(0,0,0,0.15); max-height: 90vh; overflow-y: auto;">
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 24px;">
                    <h2 style="margin: 0; color: #1f2937; font-size: 20px; font-weight: 700;">
                        <i class="fas fa-star" style="color: #f59e0b; margin-right: 8px;"></i>Appoint as Adviser
                    </h2>
                    <button onclick="closeAppointModal()" style="background: none; border: none; font-size: 24px; cursor: pointer; color: #9ca3af; padding: 0;">×</button>
                </div>
                
                <!-- Teacher Info -->
                <div style="background: #f9fafb; border-left: 4px solid #10b981; padding: 16px; border-radius: 6px; margin-bottom: 24px;">
                    <p style="margin: 0; color: #1f2937; font-weight: 600; font-size: 16px;">${name}</p>
                    <p style="margin: 8px 0 0 0; color: #6b7280; font-size: 14px;">Appoint to a class with available students</p>
                </div>

                <!-- Class Selection -->
                <div style="margin-bottom: 24px;">
                    <label style="display: block; font-size: 14px; font-weight: 600; color: #374151; margin-bottom: 12px;">Select Class <span style="color: #ef4444;">*</span></label>
                    <select id="classSelect" style="width: 100%; padding: 12px 14px; border: 2px solid #e5e7eb; border-radius: 8px; font-size: 14px; background: white; cursor: pointer; transition: all 0.2s;">
                        ${classOptions}
                    </select>
                </div>

                <!-- Info Message -->
                <div style="background: #eff6ff; border-left: 3px solid #3b82f6; padding: 12px; border-radius: 6px; margin-bottom: 24px; font-size: 13px; color: #1e40af;">
                    <i class="fas fa-info-circle" style="margin-right: 8px;"></i>
                    Only classes without advisers are shown above. The teacher will become the adviser for the selected class.
                </div>

                <!-- Buttons -->
                <div style="display: flex; gap: 12px; justify-content: flex-end;">
                    <button onclick="closeAppointModal()" style="padding: 12px 24px; border: 1px solid #e2e8f0; background: white; border-radius: 6px; cursor: pointer; font-weight: 600; color: #334155; font-size: 14px; transition: all 0.2s;" onmouseover="this.style.background='#f1f5f9'; this.style.borderColor='#cbd5e1'" onmouseout="this.style.background='white'; this.style.borderColor='#e2e8f0'">Cancel</button>
                    <button onclick="confirmAppointment(${id}, '${name}')" style="padding: 12px 24px; background: #10b981; color: white; border: none; border-radius: 6px; cursor: pointer; font-weight: 600; font-size: 14px; display: inline-flex; align-items: center; gap: 8px; transition: all 0.2s; box-shadow: 0 2px 8px rgba(16, 185, 129, 0.2);" onmouseover="this.style.background='#059669'; this.style.boxShadow='0 4px 12px rgba(16, 185, 129, 0.3)'; this.style.transform='translateY(-2px)'" onmouseout="this.style.background='#10b981'; this.style.boxShadow='0 2px 8px rgba(16, 185, 129, 0.2)'; this.style.transform='translateY(0)'">
...
function closeAppointModal() {
    const modal = document.getElementById('appointAdviserModal');
    if (modal) {
        modal.remove();
...
function confirmAppointment(id, name) {
    ...
    // Parse grade and section from the combined value
    const [gradeLevel, classSection] = classSelect.split('|');
    ...
    .then(data => {
        if (data.success) {
            alert(`${name} has been successfully appointed as adviser for Grade ${gradeLevel} - Section ${classSection}`);
            closeAppointModal();
            // Refresh the page to show updated adviser in list
            setTimeout(() => {
                location.reload();
            }, 500);
        } else {
            alert('Error: ' + (data.message || 'Failed to appoint adviser'));
            btn.disabled = false;
    ...
    .catch(error => {
        ...
        alert('Error appointing adviser: ' + error.message);
        btn.disabled = false;
        btn.textContent = 'Confirm Appointment';
    });
}
