...
// Determine which table to use (admin or teacher)
$userTable = 'teacher';
$checkTable = $conn->prepare("SELECT id FROM admin WHERE id = ?");
if ($checkTable) {
    $checkTable->bind_param("i", $teacherId);
    $checkTable->execute();
...
$checkEmail = $conn->prepare("SELECT id FROM $userTable WHERE email = ? AND id != ?");
if (!$checkEmail) {
    if ($isJson) {
...
$getCurrentData = $conn->prepare("SELECT image, password FROM $userTable WHERE id = ?");
if (!$getCurrentData) {
    if ($isJson) {
        http_response_code(500);
        echo json_encode(['success' => false, 'message' => 'Database error: ' . $conn->error]);
...
$stmt = $conn->prepare("UPDATE $userTable SET fname = ?, mname = ?, lname = ?, email = ?, password = ?, image = ? WHERE id = ?");
if (!$stmt) {
    if ($isJson) {
        http_response_code(500);
